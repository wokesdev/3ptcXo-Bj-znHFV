...
(function ($) {
    ...
    let initShowMoreLess=function(data){
        ...
        if(data.settings.lessAtInitial){
            ...
            if (($thatOgHeight / $thatLineHeightInPx) > restrictedLine) {
                $span.html(data.settings.showMoreText);
                $span.attr('class','show-more-less-handler');
                $span.attr('style',' background: rgb(11,12,42);background: linear-gradient(90deg, rgba(255,255,255,0) 0%, rgb(11,12,42) 17%, rgb(11,12,42) 100%); ');
                $span.css({
                    'position':'absolute',
                    'right':'0',
                    'bottom':'0',
